...
#define SLEEP_TIME 15 * 60 * uS_TO_S_FACTOR  // 15 minutos em microsegundos
...
String estacao_ID = "x";
String KeyValidator = "seuKeyValidator";
...
const int kNetworkTimeout = 10 * 60 * 1000;  // timeout requisição http 10 minutos
const int kNetworkDelay = 1000;
const char kHostname[] = "192.168.4.1";                // IP do servidor host
const char kPath[] = "/transferir?arquivo=/data.txt";  // Caminho para download (a parte da URL após o nome do host)
...
bool RealizaDownloadHTTP() {
  ...
  for (int tentativas = 0; tentativas < MAX_HTTP_RETRIES; tentativas++) {
    ...
    if (statusCode >= 0) {
      ...
      const long totalSizeToKeep = 10240;  // Últimos KB que você deseja manter // 10KB
      long sizeToDiscard = 0;              // Quantos bytes devem ser descartados

      if (bodyLen > totalSizeToKeep) {  // Se o tamanho do corpo for maior que o tamanho total a ser mantido
        sizeToDiscard = (bodyLen - totalSizeToKeep);
      }
      long discarded = 0;  // Quantos bytes já foram descartados

      Serial.print("Irá Descartar: ");
        ...
        while ((http.connected() || http.available()) && ((millis() - timeoutStart) < kNetworkTimeout)) {
          if (http.available()) {
            ...
            if (discarded + retornoBytes <= sizeToDiscard) {
              ...
              // Serial.print("Descartando: ");  // Serial.println(discarded);
            } else {
              ...
              if (discarded < sizeToDiscard) {
                ...
                file.flush();  //o flush é chamdo junto ao close como diz na documentação, porém não está funcionando fora do loop, então tive que forçar a gravação
...
void setup() {
  ...
  delay(2000);  // Espere um pouco para que o monitor serial inicie corretamente
...
void loop() {
  ...
  if (!RealizaDownloadHTTP()) {
    ...
    Serial.println("Download realizado com sucesso.");
  }
